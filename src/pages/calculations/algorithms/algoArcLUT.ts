import { makeArcLut } from './lutMakeArc'
import { LutPoint, getLutPoint, getMaxC } from './lutMakeArc'
...
function makeLutGetter() {
  ...
  return function getCusps(gamut: 'srgb' | 'display-p3'): LutPoint[] {
    if (!cachedLuts[gamut]) {
      cachedLuts[gamut] = makeArcLut(gamut, SLICES)
    }
