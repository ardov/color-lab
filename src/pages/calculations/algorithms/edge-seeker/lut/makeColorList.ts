...
export function makeColorList(
  ...
  const hueRegions: [number, number][] = [
    [0, 60], // red-yellow
    [60, 120], // yellow-green
    [120, 180], // green-cyan
    [180, 240], // cyan-blue
...
function hslToRgb(h: number, s: number, l: number): [number, number, number] {
  ...
  if (h < 60) return [m1, m2, 2 * l - m1]
  if (h < 120) return [m2, m1, 2 * l - m1]
  if (h < 180) return [2 * l - m1, m1, m2]
  if (h < 240) return [2 * l - m1, m2, m1]
  if (h < 300) return [m2, 2 * l - m1, m1]
  return [m1, 2 * l - m1, m2]
}
...
function filterInterpolatableColors(colors: OKLCH[], treshold = 0.00001) {
  ...
  while (changed) {
    const filtered = result.filter((color, idx, list) => {
      ...
      const interpolated = lerpColorByHue(list[idx - 1], list[idx + 1], color.h)
      const quadDistance =
